...
module.exports = function(key, email, password) {
	...
	// Create HTTP aliases
	_parent.get = function(url, data, callback, secure) {
		return _parent.sendRequest(url, 'GET', data, secure, callback);
	};
	_parent.post = function(url, data, callback, secure) {
		return _parent.sendRequest(url, 'POST', data, secure, callback);
	};
	_parent.put = function(url, data, callback, secure) {
		return _parent.sendRequest(url, 'PUT', data, secure, callback);
	};
	_parent.del = function(url, data, callback, secure) {
		return _parent.sendRequest(url, 'DELETE', data, secure, callback);
	};
	...
	_parent.sendRequest = function(url, method, data, secure, callback) {
		_parent.login(secure, function() {
			...
			_parent.urlReq(url, {
				...
				return callback(JSON.parse(body), res);
			...
			_parent.urlReq(url, {
				...
				if (res.statusCode === 200 || res.statusCode === 201) {
					...
					for (var k in cookies) {
						if (cookies[k].indexOf('_session_id') !== -1) _parent.session_cookie = cookies[k];
					}
					...
					// fail
					console.error('login error: ' + res.statusCode);
					console.error(res);
				}
